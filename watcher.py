...
# This is the Mt. Whitney permit product ID used for the permit page,
# divisions lookup, and reservation links.
PERMIT_ID = "233260"

# This is the Inyo availability inventory ID used by the availabilityv2 API.
...
# Re-remind if a slot is still open after this many minutes.
...
# every ~5h45m. This makes the script exit cleanly a little before that job's
# timeout so the handoff is graceful.
...
def fetch_divisions(permit_id):
    ...
    {"1234": "Mt. Whitney Trail - Overnight",
     "1235": "Mt. Whitney Zone - Day Use"}
...
def fetch_availability_month(permit_id, year, month):
    """
    One calendar month of availability.

    Recreation.gov 400s if you ask for a range spanning more than one month,
    so callers must chunk by month.

    NOTE:
    The availabilityv2 endpoint uses AVAILABILITY_ID (233262), not the
    Mt. Whitney permit product ID (233260).
    """
    start_date, end_date = month_bounds(year, month)

    url = (
        f"https://www.recreation.gov/api/permitinyo/{AVAILABILITY_ID}/availabilityv2"
...
def fetch_availability(permit_id, watch_dates):
    ...
    months_needed = sorted(
        {(int(d[:4]), int(d[5:7])) for d in watch_dates}
    )
...
def extract_remaining(availability_by_date, division_id, target_date):
    """
    Pull the 'remaining spots' number for one division on one date.

    This is the one function to check first if Recreation.gov changes their
    API shape — run with --debug to see the raw JSON and adjust the key names
    below to match.
    """
...
def check_once(divisions_by_type, already_alerted):
    ...
    try:
        ...
        log.error(
            "Availability fetch failed (will retry next cycle): %s",
            e
        )
        return
    ...
    for target_date in WATCH_DATES:
        ...
        for div_id, (ptype, _name) in divisions_by_type.items():
            ...
            if remaining is not None and remaining >= PARTY_SIZE:
                ...
                if (
                    first_seen is None
                    or time.time() - first_seen > reminder_seconds
                ):
                ...
                already_alerted[key] = already_alerted.get(
                    key,
                    time.time()
                )
...
def main():
    ...
    try:
        ...
        log.error(
            "Could not fetch divisions (%s). Recreation.gov's API may "
            "have changed — try --debug.",
            e,
        )
        sys.exit(1)
    ...
    log.info(
        ...
        {k: v[1] for k, v in divisions_by_type.items()}
    )

    log.info(
        "Watching dates: %s (need >= %d spot(s))",
        WATCH_DATES,
        PARTY_SIZE,
    )
    ...
    while True:
        ...
        if time.time() - start_time > MAX_RUNTIME_SECONDS:
            log.info(
                "Max runtime reached — exiting cleanly so the next "
                "scheduled run can pick up."
            )
            break
